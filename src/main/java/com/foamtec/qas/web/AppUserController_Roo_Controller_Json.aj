...
import com.foamtec.qas.web.AppUserController;
...
privileged aspect AppUserController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> AppUserController.jsonFindAppUsersByUsernameEquals(@RequestParam("username") String username) {
        HttpHeaders headers = new HttpHeaders();
        try {
            headers.add("Content-Type", "application/json; charset=utf-8");
            return new ResponseEntity<String>(AppUser.toJsonArray(AppUser.findAppUsersByUsernameEquals(username).getResultList()), headers, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<String>("{\"ERROR\":"+e.getMessage()+"\"}", headers, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
